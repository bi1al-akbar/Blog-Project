...
mongoose.connect("mongodb+srv://admin-bilal:test123@atlascluster.kvhg4ya.mongodb.net/BlogDB");
...
Article.findOne({ title: "home" }).exec((err: any, result: any): any => {
	if (err) {
		console.log(err);
	}
	if (result) {
...
async function handlingGetRequest(page: string, req: any, res: any) {
	try {
...
async function handlingPostRequest(page: string, req: any, res: any, newPost: object) {
	try {
...
app.get("/compose", (req: any, res: any) => {
	res.render("compose");
});
...
app.post("/posts", async (req: any, res: any) => {
	...
	try {
		const query = await Article.findOneAndUpdate(
			{ page: "home" },
			{ $pull: { posts: { title: req.body.deleteButton } } },
			{ new: true }
		);
		console.log(query);
		res.redirect("/");
